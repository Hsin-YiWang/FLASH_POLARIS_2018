        ...
        for i in range(DataRow+1):
            ...
            if i == 0 :
                ...
                if fileName == 'Total_profile' :
                    ...
                    out_string += '\nX SetScale/P x {0:g},{1:g}, \"Shots\",  '.format(start_at, delta) + 'Intensity_2D_' + runType + '%03d' % run #Kinetic Energy [eV]
                    out_string += '\nX NewImage/K=0 Intensity_2D_' + runType + '%03d' % run
                    out_string += '\nX ModifyImage Intensity_2D_' + runType + '%03d ctab= {*,*,PlanetEarth,0}' % run
                else :
                    out_string += 'END\n\nX SetScale/P y 0,1, \"Pixels\", Intensity_2D_Bin_' + runType + '%03d' % run
                    out_string += '\nX SetScale/P x {0:g},{1:g}, \"Time delay stage position [ps]\",  '.format(start_at, delta) + 'Intensity_2D_' + runType + '%03d' % run
                    out_string += '\nX NewImage/K=0 Intensity_2D_Bin_' + runType + '%03d' % run
                    out_string += '\nX ModifyImage Intensity_2D_Bin_' + runType + '%03d ctab= {*,*,PlanetEarth,0}' % run
                out_string += '\nX ModifyGraph nticks(top)=20,minor=1,sep(top)=10'
                out_string += '\nX ModifyGraph margin(left)=40,margin(right)=120,margin(top)=40'
                out_string += '\nX ModifyGraph lblMargin=10,lblLatPos=1'
                if fileName == 'Total_profile' :
                    out_string += '\nX ColorScale/C/N=text0/F=0/B=1/A=LT frame=2.00,frameRGB=(0,0,0),image=Intensity_2D_' + runType + '%03d,nticks=10,minor=1' % run
                else :
                    out_string += '\nX ColorScale/C/N=text0/F=0/B=1/A=LT frame=2.00,frameRGB=(0,0,0),image=Intensity_2D_Bin_' + runType + '%03d,nticks=10,minor=1' % run
                out_string += '\nX ColorScale/C/N=text0/X=0.90/Y=4.50, widthPct=0, heightPct=100\n'
